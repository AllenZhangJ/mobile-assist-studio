...
当前 V4.0 final acceptance 报告已具备命令级下一步：`npm run v4:acceptance-audit` 会在 Markdown 和 JSON 中写出缺 Android、缺 iOS、缺截图、full smoke 未过和终验重跑时对应的安全命令。该报告仍只做本地审计和留档，不执行真实设备动作。
