...
当前 Mac App Monitor 已接入 V4 验收卡：它只从 Runtime 快照展示当前平台、本地运行数量、问题数量和下一步短提示，并提供 full smoke、密码版 full smoke、Android smoke 和 acceptance audit 的命令复制入口。该入口不直接启动真机动作、不扫描 smoke 文件、不替代最终验收门禁；Android 真机现场冒烟留档仍是当前 V4.0 未完成项。
