...
- 读取最新 V4 final acceptance 报告的脱敏摘要，为 Monitor 提供终验状态、Batch 0-8 进度、iOS / Android 留档数量、iOS / Android 本机状态、结构化终验门禁、现场补验清单和下一步；不暴露报告路径、stdout、stderr 或截图内容，补验命令只保留项目内 V4 smoke / 终验白名单。
...
- `runtime_v4_acceptance_models.dart`：最新 V4 final acceptance 脱敏摘要、Batch 0-8 批次进度、结构化终验门禁和现场补验清单。
...
- `runtime_v4_acceptance_commands.dart`：最新 V4 final acceptance 摘要刷新。
...
- `v4_acceptance_store.dart`：最新 V4 final acceptance JSON 摘要读取、坏文件跳过、文本二次脱敏和补验命令白名单过滤。
...
- `test/runtime_v4_acceptance_test.dart`：V4 final acceptance 摘要读取、结构化门禁 / 补验清单解析、命令白名单、坏文件跳过和 Runtime snapshot 刷新。
