...
`npm run v4:smoke:full` 是最终现场验收入口，会先做只读前置检查，再顺序执行 iOS 与 Android 的真实 Tap、Swipe、Input 和基础 Project DSL workflow，并在最后生成 readiness / completion audit 留档。`npm run v4:smoke:full:dry-run` 只展示命令，不执行真实动作。
