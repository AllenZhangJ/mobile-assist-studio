...
当前 V4.0 full smoke 编排已支持自动准备：`npm run v4:smoke:full` 默认先用 Dart Runtime 的 Appium 进程管理能力启动或复用本机驱动，并在 FULL_SMOKE JSON / Markdown 中写入“自动准备”区块；iOS 18+ 隧道需要系统授权时仍不绕过限制，可先通过 Mac App 一键连接，或使用 `npm run v4:smoke:full:password-stdin` 从 stdin 一次性传入 Mac 密码启动本次 smoke 托管隧道。密码不得进入日志、报告、evidence 或复制内容；readiness / archive 会把自动准备阻断与前置检查阻断合并展示。
