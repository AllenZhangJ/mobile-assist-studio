...
当前 V4.0 final acceptance 报告已具备现场摘要和命令级下一步：`npm run v4:acceptance-audit` 会在 Markdown 和 JSON 中嵌入最新 readiness / archive 的本机状态摘要、截图 / iOS / Android / full smoke 留档数量，并写出缺 Android、缺 iOS、缺截图、full smoke 未过和终验重跑时对应的安全命令。该报告仍只做本地审计和留档，不执行真实设备动作，不读取截图内容。
