...
当前 V4.0 full smoke 编排已支持自动准备：`npm run v4:smoke:full` 默认先检查 Appium `xcuitest` / `uiautomator2` 平台 driver，再用 Dart Runtime 的 Appium 进程管理能力启动或复用本机驱动，检查 iOS 必要隧道，并启动 ADB server 检查 Android 是否只有一台已授权手机；`npm run v4:ios-smoke:full` 和 `npm run v4:android-smoke:full` 也复用同一编排器，只跳过另一个平台，单平台排障时同样具备自动准备能力。准备结论会写入 FULL_SMOKE JSON / Markdown 的“自动准备”区块；Android 纳入 full smoke 且前置阻断时，还会同步写入同 schema 的 `ANDROID_SMOKE_PREFLIGHT` 脱敏诊断，避免 readiness / acceptance 展示旧 Android 阻断。iOS 18+ 隧道需要系统授权时仍不绕过限制，可先通过 Mac App 一键连接，或使用 `npm run v4:smoke:full:password-stdin` 从 stdin 一次性传入 Mac 密码启动本次 smoke 托管隧道。密码不得进入日志、报告、evidence 或复制内容；Android 准备不创建 Appium session、不点击设备、不读取隐私内容；readiness / archive 会把自动准备阻断与前置检查阻断合并展示。

当前 Mac App Monitor 已接入 V4 验收卡：它只从 Runtime 快照展示当前平台、本地运行数量、问题数量和下一步短提示，并提供 full smoke、密码版 full smoke、Android smoke 和 acceptance audit 的命令复制入口。该入口不直接启动真机动作、不扫描 smoke 文件、不替代最终验收门禁；Android 单平台 smoke 在驱动或手机条件不足时会写入 `ANDROID_SMOKE_PREFLIGHT` 脱敏诊断并被 readiness 索引，但不计作真实 Android run；Android 真机现场冒烟留档仍是当前 V4.0 未完成项。
