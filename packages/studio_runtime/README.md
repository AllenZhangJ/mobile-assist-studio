...
- 读取最新 V4 final acceptance 报告的脱敏摘要，为 Monitor 提供终验状态、Batch 0-8 进度、iOS / Android 留档数量、Android 本机状态和下一步；不暴露报告路径、stdout、stderr 或截图内容。
...
- `runtime_v4_acceptance_models.dart`：最新 V4 final acceptance 脱敏摘要和 Batch 0-8 批次进度。
...
- `v4_acceptance_store.dart`：最新 V4 final acceptance JSON 摘要读取和坏文件跳过。
...
- `test/runtime_v4_acceptance_test.dart`：V4 final acceptance 摘要读取、坏文件跳过和 Runtime snapshot 刷新。
